...
const DocumentCardExtension = ({
  extension,
  ...rest
}: DocumentCardExtensionProps) => {
  return (
    <div {...rest}>
...
const DocumentCardInfo = ({
  ...
  return (
    <div
      {...rest}
      className={`${rest.className} flex flex-col ${
      ...
      {createdAt && (
        ...
        >
          ...
          {fileSize && (
            <div
              title={`Size: ${fileSize}`}
              className={`flex items-center justify-center gap-1 font-semibold text-foreground/70`}
            >
              <HardDrive size={11} />
              <span>{bytesToMBFormatted(fileSize)}</span>
            </div>
          )}
        </div>
      )}
